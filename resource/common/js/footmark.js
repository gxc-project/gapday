...
//分享的工具类(将 div内容生成一张图片，然后调用移动端share SDK 分享出去 )
var STool = {
	
	//生成点亮足迹的卡片的图片，分享出去
	shareCard : function (ele,shareCardDivId){
		FM.tipMask('正在分享...',true);
		if(ele){
			$(ele).hide();
		}
		setTimeout(function(){
			//生成一个临时的div，调整div的大小，以及div子元素的样式，div越大生成的图片会越清晰
			var document = $('#'+shareCardDivId);
			var tempdiv = $('<div/>');
			var divwidth = 589;
			var divheight = 333;
			$(tempdiv).css({'display':'block','position':'absolute','z-index':'-999999','width':divwidth+'px'});
			$('body').append(tempdiv);
			var doc = $(document).clone().appendTo(tempdiv);
			$(doc).removeClass('cd-timeline-content');
			$(doc).css({'width':divwidth+'px','height':divheight+'px'});
			var sharecardpng = $(doc).children();
			$(sharecardpng).removeClass('sharecardpng');
			$(sharecardpng).css({width:divwidth+'px',height:divheight+'px','max-width':'1024px'});
			var shareCardDiv_sub = $(sharecardpng).next();
			$(shareCardDiv_sub).removeClass('shareCardDiv_sub');
			$(shareCardDiv_sub).css({'margin-top': '-329px','margin-left':'auto','margin-right':'auto','width':'100%','max-width':'1024px'});
			var shareCardButton = $(shareCardDiv_sub).children();
			var face_div =  $(shareCardButton).next();
			$(face_div).first().css({'width':'122px','height':'82px'});
			var shareCardText = $(face_div).next();
			$(shareCardText).css({'margin-left': 'auto','margin-right':'auto','width':'95%','text-align':'center','font-size':'13px','margin-top':'10px','color': '#fff'});
			var card_title = $(shareCardText).children();
			$(card_title).css({'font-size': '34px','color': '#fff5c6','font-weight': 'bold','padding': '20px 0px 5px 0px'});
			var content_div = $(card_title).next();
			var card_content = $(content_div).children();
			$(card_content).css({'font-size':'23px','color':'#fff5c6'});
			var card_location = $(card_content).next();
			$(card_location).css({'font-size': '28px','color': '#F2CA46','font-weight': 'bold'});
			var card_content2 = $(card_location).next('.card_content');
			$(card_content2).css({'font-size': '23px','color': '#fff5c6'});
			var card_location2 = $(card_content2).next();
			$(card_location2).css({'font-size': '28px','color': '#F2CA46','font-weight':'bold'});
			var card_content3 = $(card_location2).next();
			$(card_content3).css({'font-size': '23px','color': '#fff5c6'});
			doc = $(tempdiv)[0];
			try{
				//修改html2canvas源码的renderDocument方法中的第二三个参数，改为传自定义的屏幕高宽度
				setTimeout(function(){
					//dom转canvas
					html2canvas(doc,{width:divwidth,height:divheight,windowWidth:1280,windowHeight:1024}).then(function(canvas) {
				 		var  pic = canvas.toDataURL('image/png').replace(/^data:image\/(png|jpeg);base64,/, "");//图片转为base64码
				        bridgeG.callHandler('shareFootmarkMap',pic);//调用移动端的分享方法
				        FM.tipMask();
						$(canvas).remove();
						if(ele){
							$(ele).show();
	...
	shareMap : function(){
		//FM.tipMask('正在分享...',true);
		setTimeout(function(){
			var doc,tempmapdiv = $('<div/>');
			//创建一个隐藏的地图，地图宽高越大分享出的图片越清晰
			$(tempmapdiv).css({
								'display' : 'block',
								'position' : 'absolute',
								'z-index' : '-999999',
			...
			$(tempmapdiv).attr('id','tempmapdiv');
			$('body').append(tempmapdiv);
			if(FM.currentLevel == 'world' || FM.currentLevel == 'china'){
				var area = 'world';
				if(FM.currentLevel == 'china'){
					area = 'cn';
				}
				$(tempmapdiv).addClass('jvectormap jvectormap-merc');
				$('#tempmapdiv').vectorMap({//世界或中国的地图
			    	map: area+'_merc',
			    	backgroundColor: 'transparent',
			    	regionStyle: {
			    		initial: {
				...
				$("[data-code='"+FM.currentProvince+"']").attr('fill','#F2CA46');
				$("[data-code='"+FM.currentCity+"']").attr('fill','#F2CA46');
				doc = $('#tempmapdiv > .jvectormap-container > svg')[0];
			}else if(FM.currentLevel == 'province'){
				var mapRegion = $('#tempmapdiv').SVGMap({//省级地图
					mapName: FM.currentOperProvince,
					mapWidth: 1280,
					mapHeight: 1024,
					stateInitColor: '#DBD9DA',
					stateHoverColor:'#DBD9DA',
				...
				$("[data-code='"+FM.currentProvince+"']").attr('fill','#F2CA46');
				$("[data-code='"+FM.currentCity+"']").attr('fill','#F2CA46');
				doc = $('#tempmapdiv > .svggroup > svg')[0];
				mapRegion = null;
			}
			try{
				var s = (new XMLSerializer()).serializeToString(doc);
		  		var canvas = document.createElement("canvas");
		  		//backgroundImg 背景图片，需要则写图片完整路径 backgroundColor 背景颜色
	        	canvg(canvas, s,{backgroundImg:false,backgroundColor:'#000', log: false,renderCallback: function (dom) {
		        	var pic = canvas.toDataURL('image/png').replace(/^data:image\/(png|jpeg);base64,/, "");
		        	/*$.ajax({
		                type: 'POST',
		                url: "http://wangzg:8090/lkapp/app/footmark/mapdict/savepic.do", 
		                data:{
		    		    	 pic : pic
		    		    },  
		                dataType:'jsonp',
					...
					bridgeG.callHandler('shareFootmarkMap',pic);//调用移动端的分享足迹方法
					FM.tipMask();
					$(canvas).remove();
					$(tempmapdiv).remove();
				}}); 
	...
	//将地图作为H5页面   分享出去
	shareH5Page : function(requestUrl){
		//FM.tipMask('正在分享...',true);
		setTimeout(function(){
			try{
				bridgeG.callHandler('shareMapPageView',requestUrl);  //调用移动端的分享页面的方法
			    FM.tipMask();
			}catch(e){
				FM.tipMask();
				FM.tipMask('分享失败啦!',false);
...
var LFM  = {
	...
	interval:0,//编辑卡片div光标闪烁间隔
	type:0,//1：我放飞的 2：我摇到的叶子列表 3：我摇到的叶子卡片
	uid:userid,//点击头像进入个人中心的用户id
	isEditLeef:false,//是否正在编辑叶子
	isLeefList:false,//是否在叶子列表页面
	currentCountryName:FM.currentCountryName,
	currentProvinceName:FM.currentProvinceName,
	currentCityName:FM.currentCityName,
	//打开留言树
	openMessageTree:function(){
		//alert("打开留言树");
		$('.messagetree_div').css('display','block');
		$('.openmessagetree').css('display','none');
		...
		FM.currentLevel = 'messagetree';
		var istip = window.localStorage.getItem('shakehand');
	//	if(!istip && istip!=1){
		if(istip!=1){
			$('.shakehand').css('display','block');
			LFM.shake();
		}
		if(isShare == 1){  //分享操作,摇一摇去掉
		   $('.shakehand').css('display','none');
		}
	},
	//初始化上拉刷新（叶子列表数据）
	init:function(){
		...
		$('.footmarkleeflists_timeline').height(FM.height-65);
		LFM.dropload = $('#footmarkleeflists').dropload({
			scrollArea : window,
		    domUp : {
		    },
		    domDown : {
		    },
		    loadUpFn : function(me){  //上滑   加载上一页数据
		        LFM.loadData('loadUp');
		    	me.resetload();//重置
		    },
		    loadDownFn : function(me){  //下滑  加载下一页数据
		        LFM.loadData('loadDown');
		    	me.resetload();
		    }
		});
...
//       }); 
	...
	//加载叶子列表数据
	loadData : function (oper){
		if(oper == 'loadDown'){//下拉操作
			LFM.currentPage = LFM.loadedData['currentPage'];
		//	LFM.getLeef(1);//获取我在当前城市放飞的叶子
			LFM.getLeef(LFM.type);
		}else if(oper == 'loadUp'){ //上划操作
		    LFM.currentPage = LFM.currentPage-2;  //页数减少
		    if(LFM.currentPage <0){
		    	LFM.currentPage = 1;
		    }
			LFM.getLeef(LFM.type);
		}
	...
	//我的叶子列表
    myLeef:function(){
		$('#footmarkleeflists').css('display','block');
		$('#footmarkleeflists_background').css({'display':'block','min-height':FM.height+50,'min-width':FM.width,'height':'100%','width':'100%'});
    	$('#myleef').css('display','none');
    	FM.currentLevel = 'footmarkleeflist';
    	LFM.currentPage = 1;//重置分页
    	LFM.type = 0;
    	LFM.isLeefList = true;
    	$('#getSelfLeef').attr('src',resour_ctx+'message/fly_1.png');
  		$('#getShakedLeef').attr('src',resour_ctx+'message/down_2.png?v=1.0.0');
	...
	//		return;
	//	}
		LFM.dropload.isLock = false;
		var country = FM.selectRegion['country'];
		var province = FM.selectRegion['province'];
		var city = FM.selectRegion['city'];
		$('.message_content').remove();
		FM.tipMask('正在加载数据...',false);
		$.ajax({
            type: 'GET',
            url:ctx+"/app/footmark/footmarkleef/getLeef.do", 
            data:{
		    	 userid : userid,
		    	 type:type,
		    	 country:country,
		    	 currentPage:LFM.currentPage,
		    	 showcount:LFM.showcount,
		    	 province:province,
		    	 city:city
		    },  
            dataType: 'json',
            success: function(result){
            	if(result.success){
            		if(type==3){//摇一摇   摇到的叶子（一片）
        				var obj = result.data;
        				if(obj){
        					var createtime = obj.createtime;
            				var time = createtime.split(' ');
        					createtime = time[0].replace(/\-/g,".");
            				$('.messageTipText').css('display','none');
            				$('#ciclebutton').css('display','none');
            		    	if(FM.height<500){
            		    		$('.sendText').css({'line-height':'18px','font-size':'10px','height':'60px'});
            		    	}
            		    	$('.sendText').val(obj.content);
            		    	$('.sendText').attr('readonly','readonly');
            		    	var userHeadImagePath = obj.userHeadImagePath.replace('.','_thumb_200.');
            		    	$('#messageUserHeadface').attr('src',pic_ctx+userHeadImagePath);
            	    		$('#messageUserNickname').text(obj.userNickname);
            	    		$('#messageLocation').text(obj.country+"  "+obj.city);
            	    		$('#textDate').text(createtime);
            	    		LFM.uid = obj.userid;
            	    		FM.tipMask();
        					...
        					FM.tipMask();
        					FM.tipMask('没有摇到叶子哦！再试一试吧！',false);
        					setTimeout(function(){
        						FM.tipMask();
        					},800);
        				}
            		}else{//type=1  获取 我自己放飞的叶子列表,type=2 摇下的叶子列表
                		...
                		if(length>0){
                			...
                			for(var i=0;i<length;i++){
                				...
                				var createtime = obj.createtime;
                				//自己的头像、昵称
                				var headimgpath = userHeadImgPath;
                				var nickname = userNickname;
                				if(type == 2){
                					//别人的头像、昵称
                					var hip = obj.userHeadImagePath.replace('.','_thumb_200.');
                					headimgpath = source+hip;
                					nickname = obj.userNickname;
                				}
                				var time = createtime.split(' ');
            					createtime = time[0].replace(/\-/g,".");
                				var html = '<div class="message_content message_content_1">'+
    							            	'<div class="message_content_2">'+
    							            		'<div class="message_content_3">'+
    								            		'<div class="message_content_4"></div>'+
    													'<div class="message_content_5">'+createtime+'</div>'+
    								            	'</div>'+
    								            	'<div class="message_content_6">'+
    								            		'<div class="message_content_7">'+
    								            			'<div class="message_content_16"><img class="message_content_8" src="'+headimgpath+'" onclick="openPersonalCenterActivity(\''+obj.userid+'\')" onerror="this.src=\''+resour_ctx+'default.png\'"></div>'+
    								            			'<div class="message_content_9">'+nickname+'</div>'+
    								            		'</div>'+
    								            		'<div class="message_content_10">'+
    								            			obj.content +
    								            		'</div>'+
    								            	'</div>'+
    												'<div class="message_content_11">'+
    													'<span class="message_content_12 message_content_15">'+title[0]+'</span>'+
    													'<span class="message_content_13 message_content_15">'+title[1]+'</span>'+
    													'<span class="message_content_14 message_content_15">'+title[2]+'</span>'+
    												'</div>'+
    											'</div>'+
    							            '</div>';
                				$('#cd-footmarkleeflist').append(html);
                			}
                			
                			if(length == LFM.showcount){
                  			  ++LFM.currentPage;
                  			  LFM.loadedData['currentPage'] = LFM.currentPage;
                  			}
                			else{
                  				//LFM.dropload.isLock = true;  //将下拉滚动刷新的禁掉
                				LFM.currentPage = 1; 
	                		    LFM.loadedData['currentPage'] = 1;
                  		    }
                			
                  			if(length>=2){  //改变左边时间轴的长度
                				var listdivheight = $('#cd-footmarkleeflist').height()+100;
                				$('.footmarkleeflists_timeline').css('height',listdivheight);
                			}
                			setTimeout(function(){
                    			FM.tipMask();
                    		},300);
	                	    ...
	                	    var msg = "没有叶子喽！";
	                		if(type == 1){
	                			if(LFM.currentPage == 1){
	                				LFM.dropload.isLock = true; //将下拉滚动刷新的禁掉
	                				msg = "您在这里没有放飞叶子哦！";
	                			}
	                        }else{
	                			if(LFM.currentPage == 1){
	                				LFM.dropload.isLock = true; 
	                				msg = "您在这里没有摇到叶子哦！";
	                			}
	                		}
	                		LFM.currentPage = 1; //重置为1
	                		LFM.loadedData['currentPage'] = 1;
                			FM.tipMask();
                		    FM.tipMask(msg,false);
                		}
            		}
            	}else{
            		FM.tipMask();
	...
	//点击头像到个人中心
	toPersonalCenterActivity:function(){
		openPersonalCenterActivity(LFM.uid);
	},
	
	
	//解决iphone手机在网页中输入框输入信息后摇动手机弹出撤销提示
	messageTextareaIframeLoaded:function(){
		var sendTextEle = $("#messageTextareaIframe").contents().find("#sendText");
		if(FM.height<500){
    		$('.messageTextareaIframe').css({'line-height':'18px','font-size':'10px','height':'60px'});
    		$(sendTextEle).css({'line-height':'18px','font-size':'10px','height':'60px'});
    	}
		setTimeout(function(){
			$("#messageTextareaIframe")[0].contentWindow.focusTextarea();
		},200);
	},
	//编辑卡片的提示
	messageTipText:function(){
		clearInterval(LFM.interval);
    	$('.messageTipText').css('display','none');
    	if(deviceType == 1){//如果是ios 
    		if($("#messageTextareaIframe").length == 0){
    			var textarea = $('#messageTextarea');
        		$('<iframe src="'+ctx+'/pages/footmark/input.html?v=1.0.0" id="messageTextareaIframe" class="sendText" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes"></iframe>').appendTo(textarea);
    		}else{
    			$('#messageTextareaIframe').css('display','block');
    			setTimeout(function(){
    				$("#messageTextareaIframe")[0].contentWindow.focusTextarea();
    			},200);
    		}
    		...
    		$('.sendText').val('');
    		$('.sendText').removeAttr('readonly','readonly');
        	$('.sendText').css('display','block');
        	$('.sendText').focus();
    	}
    },
    //关闭卡片
    messageCardClose:function(){
    	$('.messageCard').css('display','none');
    	clearInterval(LFM.interval);
    	if(LFM.isEditLeef && deviceType == 1){//如果是ios
    		$('#messageTextareaIframe').remove();
    	}else{
    		$('.sendText').val('');
    		$('.sendText').css('display','none')
    		$('.sendText').removeAttr('readonly','readonly');
    	}
    	LFM.isEditLeef = false;
    	LFM.uid = userid;
    },
    //判断编辑的内容是否为空
    isNull:function ( str ){
    	if ( str == "" || str.replace(/(^s*)|(s*$)/g, "").length ==0) return true;
    	var regu = "^[ ]+$";
    ...
    sendText:function(){
    	var content = "";
    	if(deviceType == 1){//如果是ios
    		content = $("#messageTextareaIframe").contents().find("#sendText").val();
    	}else{
    		content = $('.sendText').val();
    	}
    	content = content.replace(/(^\s*)|(\s*$)/g, "");
    	if(LFM.isNull(content)){
    		FM.tipMask('别太懒！写点什么吧！',false);
    		if(deviceType == 1){
    			$("#messageTextareaIframe").contents().find("#sendText").focus();
    		}else{
    			$('.sendText').focus();
    		}
    	}else if(content.length > 140){
    		FM.tipMask('字数限制在140个字以内哦！',false);
    		if(deviceType == 1){
    			$("#messageTextareaIframe").contents().find("#sendText").focus();
    		}else{
    			$('.sendText').focus();
    		...
    		FM.tipMask('正在放飞叶子...',true);
        	var aj = $.ajax({  
    		    url:ctx+"/app/footmark/footmarkleef/flyFootmarkLeef.do", 
    		    data:{
    		    	 userid : userid,
    		    	 content : content,
    		    	 country : LFM.currentCountryName,  
    	             province : LFM.currentProvinceName,  
    	             city : LFM.currentCityName,
    	             deviceType : deviceType
    		    },  
    		    type:'post',  
    		    dataType:'json',  
    		    success:function(result) {
    		    	...
    		    	if(result.success){
    		    		if(result.code=='102'){
    		    			...
    		    			FM.tipMask('放飞成功！',true);
    		    		}
    		    		LFM.messageCardClose();
    		    	}else{
    		    		FM.tipMask(result.msg,false);
    		    	}
    		    },  
    ...
    flyLeaf:function(){
		...
		if(isCanFly){//如果是编辑当前城市的叶子
			LFM.currentCountryName = FM.currentCountryName;
			LFM.currentProvinceName = FM.currentProvinceName;
			LFM.currentCityName = FM.currentCityName;
		}
		//历史已点亮的可以放飞叶子
		var length = FM.historyFootmark.length;
  		if(!isCanFly && length > 0){//判断当前所点击的地图是否是历史足迹已点亮的
  			for(var i=0;i<length;i++){
  				var footmark = FM.historyFootmark[i];
  				if(FM.level == 'country'){
  					if(footmark.country == city_country){
  						isCanFly = true;
  					}
  				}else if(FM.level == 'city'){
  					if(footmark.city == city_country){
  						isCanFly = true;
  					}
  				}else if(FM.level == 'china' && (city_country == 'aomen' || city_country == 'taiwan' || city_country == 'xianggang' 
  					|| city_country == 'beijing'|| city_country == 'shanghai'|| city_country == 'tianjin'|| city_country == 'chongqing')){//特别行政区域
  					if(footmark.province == city_country){
  						isCanFly = true;
  					}
  				}
  				if(isCanFly){//如果是编辑历史去过的城市的叶子
  					LFM.currentCountryName = footmark.countryname;
  					LFM.currentProvinceName = footmark.provincename;
  					LFM.currentCityName = footmark.cityname;
  					break;
  				}
  			}
  		}
		if(isCanFly){//可以放飞叶子
			$('.messageCard').css('display','block');
	    	if(FM.height<500){
	    		$('.messageTipText').css({'padding':'0px','line-height':'13px','font-size':'10px'});
	    		$('.sendText').css({'line-height':'18px','font-size':'10px','height':'60px'});
	    	}
    		$('.sendText').removeAttr('readonly','readonly');
    		$('.sendText').val('');
    		$('.sendText').css('display','none');
    		$('.messageTipText').css('display','block');
    		$('#ciclebutton').css('display','block');
	    	$('#messageUserHeadface').attr('src',userHeadImgPath);
    		$('#messageUserNickname').text(userNickname);
    		$('#messageLocation').text(LFM.currentCountryName+"  "+LFM.currentCityName);
    		var date=new Date();
    		var year = date.getFullYear();
    		var month = date.getMonth()+1;
    		var day = date.getDate();
    		$('#textDate').text(year+'.'+month+'.'+day);
    		LFM.isEditLeef = true;
    		LFM.uid = userid;
    		LFM.interval = setInterval(function(){
    			var display = $('#messageTipTextCursor').css('visibility');
    			if(display == 'hidden'){
    				$('#messageTipTextCursor').css('visibility','visible');
    			}else{
    				$('#messageTipTextCursor').css('visibility','hidden');
    			}
    		},500);
		}else{
			if(FM.level == 'china' || FM.level == 'world' || FM.level == 'province'){
				FM.tipMask('进入到您点亮的地方才能放飞叶子哦！试试摇一摇吧！',false);
			}else{
				FM.tipMask('您没有去过'+flag+',不能放飞叶子哦！试试摇一摇吧！',false);
			}
		}
    },
	...
	removeShakeEvent:function(){
		window.removeEventListener('devicemotion',LFM.deviceMotionHandler,false);
	},
	
	//摇一摇（摇到一片叶子）
	shake:function(){
	    if (window.DeviceMotionEvent) {
	        window.addEventListener('devicemotion', LFM.deviceMotionHandler, false);
	    } else {
	        FM.tipMask('您的设备不支持摇一摇功能哦！',false);
	        return;
	    }
	    //获取当天摇一摇次数
	    var aj = $.ajax({  
		    url:ctx+"/app/footmark/footmarkleef/getShakeLeafNum.do", 
		    data:{
		    	 userid : userid  
		    },  
		    type:'get',  
		    ...
		    success:function(result) {
		    	if(result.success){
		    		LFM.shakeNum = result.num;
		    	}
		    },  
		    error : function(data) {
		    }  
		});
	    LFM.lastTime = new Date().getTime();
	},
	SHAKE_THRESHOLD:800,
    last_update:0,
	...
	deviceMotionHandler:function(eventData) {
    	if(LFM.isEditLeef){//如果正在编辑
    		return;
    	}
    	if(LFM.isLeefList){//如果正在显示叶子列表
    		return;
    	}
        var acceleration = eventData.accelerationIncludingGravity;
        var curTime = new Date().getTime();
        if(LFM.last_update == 0 || (curTime - LFM.lastTime) > 3000){
        	if ((curTime - LFM.last_update) > 100) {
	            var diffTime = curTime - LFM.last_update;
	            LFM.last_update = curTime;
	            LFM.x = acceleration.x;
	            LFM.y = acceleration.y;
	            LFM.z = acceleration.z;
	            var speed = Math.abs(LFM.x + LFM.y + LFM.z - LFM.last_x - LFM.last_y - LFM.last_z) / diffTime * 10000;
	            if (speed > LFM.SHAKE_THRESHOLD) {
	            	if(LFM.shakeNum > 30){
	            		FM.tipMask('一天最多摇30片叶子哦！',false);
	            	}else{
	            		bridgeG.callHandler('shakeFromFootmark');//调用移动端震动方法
		                LFM.lastTime = new Date().getTime();
		                LFM.getLeef(3);//获取别人的留言
	            	}
	            }
	            LFM.last_x = LFM.x;
	            LFM.last_y = LFM.y;
